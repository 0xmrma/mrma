...
```bash
gh release verify v0.4.5 -R 0xmrma/mrma
gh release download v0.4.5 -R 0xmrma/mrma \
  --pattern 'mrma-0.4.5-py3-none-any.whl' \
  --pattern 'mrma-0.4.5.tar.gz'
gh release verify-asset v0.4.5 ./mrma-0.4.5-py3-none-any.whl -R 0xmrma/mrma
gh release verify-asset v0.4.5 ./mrma-0.4.5.tar.gz -R 0xmrma/mrma
gh attestation verify ./mrma-0.4.5-py3-none-any.whl \
  -R 0xmrma/mrma \
  --signer-workflow 0xmrma/mrma/.github/workflows/release.yml \
  --source-ref refs/tags/v0.4.5
gh attestation verify ./mrma-0.4.5.tar.gz \
  -R 0xmrma/mrma \
  --signer-workflow 0xmrma/mrma/.github/workflows/release.yml \
  --source-ref refs/tags/v0.4.5
```
...
```bash
gh attestation verify oci://ghcr.io/0xmrma/mrma:0.4.5 \
  -R 0xmrma/mrma \
...
```bash
docker buildx imagetools inspect ghcr.io/0xmrma/mrma:0.4.5
```
